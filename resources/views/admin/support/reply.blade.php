	...
	<div class="bg-white overflow-auto">
		...
		<div class="flex-1 p:2 sm:p-6 justify-between flex flex-col h-screen">
				...
				<div class="relative flex items-center space-x-4">
						<div class="relative">
						<img src="{{ isset($support->user->image) ? url("storage/{$support->user->image}") : url('images/user.png') }}" alt="" class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">
						</div>
						<div class="flex flex-col leading-tight">
								<div class="text-2xl mt-1 flex items-center">
										<span class="text-gray-700 mr-3">
												{{ isset($support->user->name) ? $support->user->name : ''}}
										</span>
								</div>
						</div>
				</div>
										...
										@if ($user->id == $support->user->id)
												<div class="flex items-end">
														<div class="flex flex-col space-y-2 max-w-xs mx-2 order-2">
																<div><span class="px-4 py-2 rounded-lg inline-block align-top text-justify rounded-bl-none bg-gray-300 text-gray-600">
																		{{ $reply->description }}
																</span></div>
														</div>
								...
								<div class="chat-message">
										<div class="flex items-end {{ $classeChatMessage }}">
												<div class="flex flex-col space-y-2 max-w-xs mx-2 {{ $orderText }} items-start">
														<div>
															<span class="px-4 py-2 rounded-lg inline-block rounded-bl-none {{ $bgChat }}">
																{{ $reply->description }}
															</span>
														</div>
												</div>
